...
public class SMPPClientSMSSender {
	...
	private final Logger logger = LogManager.getLogger(SMPPClientSMSSender.class.getName());
	private final Logger Errorlogger = LogManager.getLogger(ErrorHandling.class.getName());

	private byte deliveryReceipt = SmppConstants.REGISTERED_DELIVERY_SMSC_RECEIPT_NOT_REQUESTED;
	...
	private byte srcTON = SmppConstants.TON_UNKNOWN;
	private byte destTON = SmppConstants.TON_UNKNOWN;
	private byte srcNPI = SmppConstants.NPI_UNKNOWN;
	private byte destNPI = SmppConstants.NPI_UNKNOWN;

	private long timeout = SmppConstants.DEFAULT_CONNECT_TIMEOUT;
	private Boolean isPayload = false;
	private Boolean isLongSMS_SAR = false;
	...
	@PostConstruct
	public void init() {
		Boolean isDeliveryRequested = smsConfig.getSMPP_CLIENT_DELIVERY_FLAG();
		if (isDeliveryRequested) {
			deliveryReceipt = SmppConstants.REGISTERED_DELIVERY_SMSC_RECEIPT_REQUESTED;
		}
		tpsController = new ThroughputController(smsConfig.getSMPP_CLIENT_MAX_TPS());
		srcTON = utility.getTONByteValue(smsConfig.getSMPP_CLIENT_SRC_TON());
		destTON = utility.getTONByteValue(smsConfig.getSMPP_CLIENT_DEST_TON());
		srcNPI = utility.getNPIByteValue(smsConfig.getSMPP_CLIENT_SRC_NPI());
		destNPI = utility.getNPIByteValue(smsConfig.getSMPP_CLIENT_DEST_NPI());
		timeout = smsConfig.getSMPP_CLIENT_BIND_TIMEOUT();
		isPayload = smsConfig.getSMPP_CLIENT_PAYLOAD_FLAG();
		isLongSMS_SAR = smsConfig.getSMPP_CLIENT_MULTIPART_SAR_FLAG();
	}

	public String sendSMS(DefaultSmppSession session, byte generalDataCoding, int dataCodingValue, String source, String destination, String smsText, int sequenceNumber) throws Exception {

		if ((dataCodingValue == 8 && smsText.length() > 60)) {
			if (!isPayload) {
				if (isLongSMS_SAR) {
					return submitLongSMS_SAR(session, generalDataCoding, dataCodingValue, source, destination, smsText, sequenceNumber)[0];
				} else {
					return submitLongSMS_UDH(session, generalDataCoding, dataCodingValue, source, destination, smsText, sequenceNumber)[0];
				}
			} else {
				return submit(session, generalDataCoding, source, destination, smsText, sequenceNumber, true);
			}
		...
		return submit(session, generalDataCoding, source, destination, smsText, sequenceNumber, false);
	}

	private String submit(DefaultSmppSession session, byte generalDataCoding, String source, String destination, String smsText, int sequenceNumber, boolean isPayload) throws Exception {
	...
	private String[] submitLongSMS_SAR(DefaultSmppSession session, byte generalDataCoding, int dataCodingValue, String source, String destination, String smsText, int sequenceNumber) throws Exception {
		String[] msgId = null;
		int splitSize = 0;
		...
		if ((dataCodingValue == 8 && smsText.length() <= 60) || (dataCodingValue == 0 && smsText.length() <= 150)) {
			msgId = new String[1];
			msgId[0] = submit(session, generalDataCoding, source, destination, smsText, sequenceNumber, false);
			return msgId;
		}
		if (smsText != null) {
	...
	private String[] submitLongSMS_UDH(DefaultSmppSession session, byte generalDataCoding, int dataCodingValue, String source, String destination, String smsText, int sequenceNumber) {
		String[] msgId = null;
		byte[] textBytes = null;
		int splitSize = 0;
		if (dataCodingValue == 8) {
